...
JSON Web Tokens can be used to transport any kind of data. They are mainly used to transport claims. When you receive a token that contains claims, it is important to check the values of these claims.
...
{% hint style="info" %}
Time-based checkers require a [PSR-20 clock](https://www.php-fig.org/psr/psr-20/).
{% endhint %}
...
$claimCheckerManager = new ClaimCheckerManager(
    [
        new Checker\IssuedAtChecker($clock),
        new Checker\NotBeforeChecker($clock),
...
When instantiated, call the method `check` to check the claims of a JWT object. This method only accepts an associative array. You have to retrieve this array by converting the JWT payload.
...
In some cases, it could be interesting to reject tokens that do not contain some mandatory claims. A list of mandatory claims can be set as second argument. If one of those claims is missing, an exception is thrown even if the claim has not been checked.
...
In this example, we will create a class that will check the claim `foo`. The claim accepts only a string with the value `bar` or `bat`. All claim checkers must implement the interface `Jose\Component\Checker\ClaimChecker`.
...
The [RFC 7519 section 5.3](https://tools.ietf.org/html/rfc7519#section-5.3) allows replicating some claims in the header. This behaviour is very useful with encrypted tokens as it helps to reject invalid tokens without decryption of the payload.
...
This framework provides a Claim Checker Manager Factory. This factory is able to accept as many claim checkers as you need. Each claim checker you add to this factory is associated with an alias. You will then be able to create a claim checker manager using those aliases.
