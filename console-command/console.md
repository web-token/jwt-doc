...
* `--uri`: output the JWK Thumbprint URI ([RFC 9278](https://www.rfc-editor.org/rfc/rfc9278.html)) instead of the bare thumbprint. The hashing method may then be given by its IANA name (`sha-256`, `sha-512`, `sha3-256`…); the PHP names of those functions are accepted too. Since 4.3.
...
```bash
./jose.phar key:thumbprint --uri --hash sha-256 '{"kty":"RSA","n":"l4mLzvr6ewIWrPvP6j5PYp0yPRhtkMW1F-dbQ1VWGoB_Mq5IIuflOo7W2ERyh71exUGkmvoesWL3zCtFIOnlxw","e":"AQAB"}'

urn:ietf:params:oauth:jwk-thumbprint:sha-256:gNur2UtA8NMAoxJfgMYhJqnuWR8u-60aeRbKtZwj4DE
```
